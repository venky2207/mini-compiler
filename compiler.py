...
def create_c_file():
    ...
    for i in quadList:
        ...
        # assign
            ...
            cFile.write("\tL_" + str(i) + ": " + str(quadList[i][3]) + " = " + str(quadList[i][1]) + ";	//" + str(
                quadList[i]) + "\n")
        # operators
        elif (quadList[i][0] == "+" and flag == True):
            cFile.write("\tL_" + str(i) + ": " + str(quadList[i][3]) + " = " + str(quadList[i][1]) + " + " + str(
                quadList[i][2]) + ";	//" + str(quadList[i]) + "\n")
        elif (quadList[i][0] == "-" and flag == True):
            cFile.write("\tL_" + str(i) + ": " + str(quadList[i][3]) + " = " + str(quadList[i][1]) + " - " + str(
                quadList[i][2]) + ";	//" + str(quadList[i]) + "\n")
        elif (quadList[i][0] == "*" and flag == True):
        ...
        # comparisons
            ...
            cFile.write(
                "\tL_" + str(i) + ": if(" + str(quadList[i][1]) + " < " + str(quadList[i][2]) + ") goto L_" + str(
                    quadList[i][3]) + ";	//" + str(quadList[i]) + "\n")
        elif (quadList[i][0] == ">" and flag == True):
            cFile.write(
                "\tL_" + str(i) + ": if(" + str(quadList[i][1]) + " > " + str(quadList[i][2]) + ") goto L_" + str(
                    quadList[i][3]) + ";	//" + str(quadList[i]) + "\n")
        ...
        # input
            ...
            cFile.write(
                "\tL_" + str(i) + ": " + 'scanf("%d", &' + str(quadList[i][1]) + ");	//" + str(quadList[i]) + "\n")
        # output
        elif (quadList[i][0] == "out" and flag == True):
            cFile.write("\tL_" + str(i) + ": " + 'printf("%d\\n", ' + str(quadList[i][1]) + ");	//" + str(
                quadList[i]) + "\n")
        # return
    ...
    if (v.isdigit()):  # unsinged integers
        ...
        if (nstLvl == 0 and entity.entityType == "Variable"):  # v global variable in main
            ...
            asmFile.write("	lw $t" + str(r) + ", -" + str(entity.offset) + "($sp)\n")
                ...
                nstLvl == current_level and entity.entityType == "Parameter" and entity.parMode == "inout"):  # parameter passed by reference
            asmFile.write("	lw $t0, -" + str(entity.offset) + "($sp)\n")
            asmFile.write("	lw $t" + str(r) + ", ($t0)\n")
        elif (nstLvl < current_level and (entity.entityType == "Variable" or (
                entity.entityType == "Parameter" and entity.parMode == "in"))):  # local variable or parameter passed by value on a lower nesting level
            gnvlcode(v)
            asmFile.write("	lw $t" + str(r) + ", ($t0)\n")
        elif (
                nstLvl < current_level and entity.entityType == "Parameter" and entity.parMode == "inout"):  # parameter passed by reference on a lower nesting level
            gnvlcode(v)
...
def create_assembly_file(quad, name):  # main final code function
    ...
    # parameters
        ...
        if (parSerialNum == 0):  # find the framelength of the function's/procedure's parameters
            for i in range(quad,
                           quad + 999):  # if we have a function/procedure with more that 999 parameters we may have a problem!!!
        ...
        if (quadList[quad][2] == "CV"):
            ...
            if (nstLvl == current_level and (entity.entityType == "Variable" or (
                    entity.entityType == "Parameter" and entity.parMode == "in"))):  # variable or parameter passed by value
    ...
    # functions or procedures
        ...
        asmFile.write("L" + str(quad) + ":\n")
        caller_entity, caller_nstLvl = searchEntity(
            name)  # nesting level of the caller(function/procedure/program we are in)
        ...
        if (state == 0):  # state0
            ...
            if (char in whitespaces or char in specialChar):
                ...
                if (abs(int(token)) > 32767):
                    state = error
                    print("Constants must be within [-32767, 32767] range, line " + str(
                        line_num) + ". Program terminated!")
                    sys.exit(-1)
            elif (char in letters):
                state = error
...
test = sys.argv[1].split(".").pop(0)  # keeps the name of the file that we compile e.g test.min --> test
# assembly file
